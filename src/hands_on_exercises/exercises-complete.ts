 ...
 */
const groupedTests = testData1.reduce(
  (acc, curr) => {
    if (!acc[curr.status]) {
      acc[curr.status] = [];
    }
...
function generateReport(
  ...
  const passRate =
    total === 0 ? 0 : parseFloat(((passed / total) * 100).toFixed(2));
...
function processQueue(
  queue: { name: string; priority: number; estimated: number }[],
) {
